...
export function StoragePage({ volumes, loading, onRefresh, error, notify, confirmVolumeDelete = true }) {
  ...
  return <Page title="볼륨" description="컨테이너 데이터를 영구 보관하는 Docker 볼륨을 관리합니다." action={<div className="heading-actions"><button onClick={onRefresh} disabled={loading || Boolean(working)}>{loading ? "조회 중..." : "새로고침"}</button></div>}>
    {(error || actionError) && <p className="dashboard-error page-alert" role="alert">{actionError || error}</p>}
...
export function SupportPage({ runtime, lastChecked, notify }) {
  async function copy() { await navigator.clipboard?.writeText(`Container Desktop\nRuntime: ${runtime}\nLast checked: ${lastChecked}`); notify("진단 정보를 복사했습니다."); }
